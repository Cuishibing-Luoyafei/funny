...
public class NettyInvokerResponseEncoder extends MessageToByteEncoder<InvokerResponse> {
    ...
    private MessageEncoder<InvokerResponse, DefaultMessage> messageEncoder;

    public NettyInvokerResponseEncoder() {
    }

    public NettyInvokerResponseEncoder(MessageEncoder<InvokerResponse, DefaultMessage> messageEncoder) {
        this.messageEncoder = messageEncoder;
    }

    @Override
    protected void encode(ChannelHandlerContext ctx, InvokerResponse response, ByteBuf out) throws Exception {
        DefaultMessage message = messageEncoder.encode(response);
        message.write(out);
    }
